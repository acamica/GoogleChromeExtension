var trial_prop = [{},{"_id": 0, "name":"base", "description":"Define la URL base de todos los links relativos de una página web. Debe ubicarse en el <head>", "example":"<base href=http://htmlreference.io>"}, 
	{ "_id": 1, "name":"address", "description":"Define un bloque para información de contacto.", "example":"<address>\n Infinite Loop,<br>\n Cupertino, CA<br>\n 95014, USA\n</address>"}];
...
window.onload = function() {
    ...
    $( '.row.snippet-area, #faves-list .modal-body' ).on( 'click', 'a.amz, a.gr, a.lt, a.bkwty', function() {
        ...
        if( $( this ).parent()[0].id == 'book-actions' ) {
            chrome.storage.local.get( { 'cached_props': [] }, function( cb ) {
                ...
                console.log(bo)
                $.getJSON( 'db.json?bo=' + bo + '&cl=' + cl + '&callback=?' );
            });
        } else {
    ...
    *******************************************************************/
    ...
    function transform_data( json ) {
        ...
        for( var o in json ) {
            ...
            if( json[o]['supersnip_text'] ) {
                ...
                if( json[o]['supersnip_text'] ===  "<p class='visual-quote'>La versión de tu extensión de Chrome está desactualizada.<br><br>¡Actualizala por favor!</p>" ) {
                    json[o]['supersnip_text'] = null;
                }
            }
            
            var name = json[o]['name'];
        }
	...
	chrome.storage.local.get( { 'userid': '', 'gold_status': false, 'first_time': true, 'number_stumbles': 0, 'update_interval': 'minutes', 'no_trial_offer_yet': true, user_32: 'false' }, function( items ) {
        ...
        if (userid) {
			...
			if( items.gold_status ) {
				chrome.storage.local.set( { userid: userid, user_32: true, cached_props: [] }, function() {
					get_books( userid );
				});
			} else {
				//set cached_props to trial books for new users
				transformed = transform_data( trial_prop );
				chrome.storage.local.set( { userid: userid, user_32: true, cached_props: transformed }, function() {
					get_books( userid );
				});
	...
	//offer time no
	$( '#offer-time #sell-box .offer.no-pronto .offer-action' ).on( 'click', function() {
		$( '#offer-time' ).modal( 'hide' );
		chrome.storage.local.set( { 'update_interval': 'minutes', 'no_trial_offer_yet': false } );
	});
	
	//offer time yeah
	$( '#offer-time #sell-box .offer.yes-pronto .offer-action' ).on( 'click', function() {
		$( '#offer-time' ).modal( 'hide' );
		chrome.storage.local.set( { 'update_interval': 'minutes', 'no_trial_offer_yet': false } );
	});
  ...
  //   });
    ...
    function mod_the_dom( prop ) {
        
        /*chrome.storage.local.get( 'number_stumbles', function( n ) {
            $( '#stumble-number' ).text( n.number_stumbles );
        });*/
        
        $( '#book-name' ).text( prop.name );
        $( '#book-author' ).text( prop.description );
        $( '#book-info' ).html( prop.example );
